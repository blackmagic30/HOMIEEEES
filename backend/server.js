...
if(process.env.NODE_ENV === 'production') { // For running frontend if we are hosting both frontend and backend on the server. The server will serve the static build file in frontend/build
...
io.on('connection', socket => {  
    ...
    socket.on("join", ({userId, username}) => {
       ...
       socket.on("Input Group Chat Message", (msg) => {  
        const encryptedMessage = cryptr.encrypt(msg.message);
          let chat = new Chat({
              message: encryptedMessage,
              cloudinary_id: msg.cloudinary_id,
          ...
          chat.save((error, doc) => {
              ...
              .exec((err, result) => {
                ...
                result.map(r => r.message = cryptr.decrypt(r.message));
                io.sockets.emit("Output Group Chat Message", result);

              });
          });
    });

    socket.on("Delete Group Chat", (deleteChat) => {
        Chat.findOneAndDelete({_id: deleteChat.chatId, "sender": deleteChat.userId})
        .exec((error, data) => {
            if(error) {
                console.log(error);
            }
            if(data) {
                ...
                data.remove();
                data.save();
                Chat.find({group: deleteChat.groupId})
                .populate('sender')
                .exec((err, result) => {
                  if(err) {
                      console.log(err);
